...
export default function LiveChat() {
  ...
  return (
    <div
      className="absolute right-4 top-4 bottom-4 z-20 w-[320px] flex flex-col overflow-hidden"
      style={{
    ...
    >
      ...
      >
        <div className="flex items-center gap-[12px]">
          ...
          <div>
            <div className="font-bold text-[18px] text-white">Live Chat</div>
        ...
        <div className="flex flex-col gap-[16px]">
          {MOCK_MESSAGES.map((msg) => (
            <div key={msg.id} className="flex gap-[12px]">
              ...
              >
                <span className="text-[20px] leading-none">{msg.emoji}</span>
              </div>
              {/* Content */}
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-[8px] mb-[4px]">
                  <span className="font-semibold text-[14px] text-white">{msg.user}</span>
                ...
                {/* Message bubble */}
                <div
                  className="bg-[#1e2939] rounded-[16px] h-[40px] px-[16px] flex items-center"
                >
                  <span className="text-[16px] text-[#f3f4f6] truncate">{msg.text}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      >
        ...
        <input
          ...
          className="flex-1 h-[42px] bg-[#1e2939] border border-[#364153] rounded-[10px] px-[12px] text-[16px] text-white placeholder-[#6a7282] outline-none"
        />
        {/* Emoji button */}
        <button
          className="w-[38px] h-[42px] bg-[#1e2939] border border-[#364153] rounded-[10px] flex items-center justify-center text-[#6a7282] hover:text-white transition cursor-pointer"
        >
          <svg className="w-[20px] h-[20px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {/* Send button */}
        <button
          className="w-[36px] h-[42px] bg-[#155dfc] rounded-[10px] flex items-center justify-center text-white hover:brightness-110 transition cursor-pointer"
        >
