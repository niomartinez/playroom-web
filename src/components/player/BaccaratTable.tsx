...
export default function BaccaratTable() {
  ...
  // Desktop's dedicated result panel is where players actually look for the
  // outcome — the bet pad glowing alone left this, the more prominent surface,
  // completely unmarked. Read from `winner` rather than a roundStatus check so
  // it persists through the post-settlement lull instead of blinking out.
  const deskPlayerWin = winner === "P";
  const deskBankerWin = winner === "B";
  const deskTie = winner === "T";
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          borderRadius: "1vw",
          padding: "1vh 1vw",
          ...(deskPlayerWin || deskTie
            ? { animation: `${deskTie ? "prg-pad-tie" : "prg-pad-win"} 1.9s ease-in-out infinite` }
            : null),
        }}
      >
