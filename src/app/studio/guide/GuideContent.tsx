      ...
      <div className="mx-auto flex gap-8 px-6 py-6" style={{ maxWidth: 1100 }}>
        ...
        <main className="flex-1 min-w-0 guide-content" style={{ color: "#d1d5db" }}>
          ...
          <Section item={TOC[0]}>
            ...
            <div className="step"><span className="step-num">1</span><span className="step-text">Live shift on production → pick the real table for your pit: <strong>&ldquo;Baccarat Table 1&rdquo;</strong> / <strong>&ldquo;Baccarat Table 2&rdquo;</strong> (game id <code>BAC-TABLE-01</code> / <code>BAC-TABLE-02</code>).</span></div>
            ...
            <div className="info"><strong>Go by the game id, not the label.</strong> <code>BAC-TABLE-01</code> and <code>BAC-TABLE-02</code> are the real tables whatever they are currently called, and anything starting with <code>TEST-</code> is not live.</div>
            <div className="info"><strong>Players may see a different name than you do — that is normal.</strong> Your picker shows the plain studio name (&ldquo;Baccarat Table 1&rdquo;). The operator&rsquo;s lobby shows their own branded name for the same table (e.g. GameSpot &rarr; Games &rarr; PRG &rarr; &ldquo;XXX Baccarat Table 1&rdquo;). Same table, two labels — so if someone quotes a name you can&rsquo;t find, ask for the <strong>game id</strong>.</div>
          ...
          <Section item={TOC[11]}>
            <table>
              ...
              <tbody>
                ...
                <tr><td><strong>The table I was told to pick isn&apos;t in the list</strong></td><td>You were probably given the name players see, which differs from the studio label on purpose (see <a href="#which-site" style={{ color: GOLD }}>Section 1</a>). Pick by id — <code>BAC-TABLE-01</code> / <code>BAC-TABLE-02</code> are the live tables regardless of label. If you only see <code>TEST-</code> tables, you are on staging.</td></tr>
