...
/** GET /api/admin/reports/by-site — GGR per downstream site.

 *
 * Aggregates in SQL on the backend, so unlike by-operator/by-table it carries
 * no row cap and no `truncated` flag. */
export async function GET(req: NextRequest) {
  const { searchParams } = new URL(req.url);
  const qs = searchParams.toString();
  const url = `${API_URL}/internal/admin/reports/by-site${qs ? `?${qs}` : ""}`;
