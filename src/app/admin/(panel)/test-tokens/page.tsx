...
export default function TestTokensPage() {
  ...
  const rowActions = (h: HistoryRow) => (
    <div className="flex items-center gap-1.5 max-md:flex-wrap max-md:justify-end">
      {h.token ? (
        <button
          onClick={() => {
          ...
          className="text-xs px-2 py-1 rounded whitespace-nowrap max-md:min-h-[44px] max-md:px-3"
  ...
  /* Ten columns cannot be read at 390px, and side-scrolling them hides the
  ...
  const historyCards: MobileCardItem[] = history.map((h, i) => ({
    ...
    rows: [
      ...
      { label: "Table", value: h.table || "—" },
      { label: "Status", value: statusCell(h) },
      { label: "Stream", value: streamCell(h) },
      { label: "Balance", value: `₱${Number(h.balance || 0).toLocaleString()}` },
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4 rounded-lg p-5" style={{ backgroundColor: "#111", border: "1px solid #1f1f1f" }}>
        <div>
          ...
          <select
            className="w-full rounded px-3 py-2 text-white text-sm max-md:min-h-[44px]"
            style={inputStyle}
            value={table}
        ...
        {/* Four fields abreast is fine on a desktop row and unusable at 390px,
            where each would land at ~80px. Below md they stack. */}
        <div className="flex gap-4 max-md:flex-col">
          <div className="flex-1">
            ...
            <input
              ...
              maxLength={16}
              className="w-full rounded px-3 py-2 text-white text-sm max-md:min-h-[44px]" style={inputStyle}
            />
        ...
        <button
          ...
          className="rounded px-4 py-2 text-sm font-semibold text-white disabled:opacity-50 max-md:w-full max-md:min-h-[44px]"
          ...
          {tokens.map((t) => (
            <div key={t.token} className="rounded-lg p-3" style={{ backgroundColor: "#0d0d0d", border: "1px solid #1f1f1f" }}>
              <div className="flex items-center justify-between mb-1">
                ...
                <button onClick={() => copy(t.url)} className="text-xs px-2 py-1 rounded max-md:min-h-[44px] max-md:px-4" style={{ backgroundColor: "#1f2937", color: "#e5e7eb" }}>
                  Copy
                </button>
              </div>
              ...
              <tbody>
                {history.map((h, i) => (
                  <tr key={i} style={{ borderTop: "1px solid #1a1a1a" }}>
                    ...
                    <td className="px-3 py-2 whitespace-nowrap">{streamCell(h)}</td>
